...
struct cmd *split_line(char *line) {
    ...
    while (token != NULL) {
        if (token[0] == '|') {
            ...
            new_pipe->in = 3;   // 預設管道輸入
            new_pipe->out = 4;  // 預設管道輸出
            temp->next = new_pipe;
            temp = new_pipe;
        } else if (token[0] == '<') {
            token = strtok(NULL, " ");
            ...
            if (token[0] == '\"') {
                char *quoted_arg = token + 1;
                char *end_quote = strchr(quoted_arg, '\"');
                if (!end_quote) {
                    char *next_token = strtok(NULL, "\"");
                    strcat(quoted_arg, " ");
                    strcat(quoted_arg, next_token);
                } else {
                    *end_quote = '\0';
                }
                temp->args[temp->length++] = quoted_arg;
            } else {
